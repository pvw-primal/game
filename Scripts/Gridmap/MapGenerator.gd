...
func init():
	if !initialized:
		...
		for i in range(1, mesh_library.get_item_list().size()):
			mesh_library.get_item_mesh(i).surface_get_material(0).albedo_color = controller.level.color.darkened(.02 * float(i - 1))
		...
		mesh_library.get_item_mesh(num).surface_get_material(0).albedo_color = controller.level.color.darkened(.03 * randi_range(0, 1))
		s.queue_free()
	...
	for i in range(0, lastRoom + 1):
		...
		for _j in range(10):
			...
			match randi_range(0, 2):
				0:
					s = Vector3i(randi_range(r.position.x - 3, r.position.x), 1, randi_range(r.position.y, r.end.y))
				1:
					s = Vector3i(randi_range(r.position.x, r.end.x), 1, randi_range(r.end.y - 3, r.end.y))
				2:
					s = Vector3i(randi_range(r.end.x - 3, r.end.x), 1, randi_range(r.position.y, r.end.y))
...
func PlaceTileEffect(pos : Vector2i, effect : TileEffect.Effect, placer : Entity = null):
	...
	if placer == null || placer.Type != "Player":
